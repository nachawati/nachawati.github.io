...
[Tree-sitter](https://tree-sitter.github.io/tree-sitter/) is great tool for building incremental and error-tolerant parsers for use in compilers, IDEs and code analyzers, which can also run directly in the browser. Unlike [ANTLR](https://www.antlr.org/), which is a popular open-source [LL(*) parser](https://en.wikipedia.org/wiki/LL_parser) generator, tree-sitter is a [GLR parser](https://en.wikipedia.org/wiki/GLR_parser) generator that's similar but more powerful than the [LALR parser](https://en.wikipedia.org/wiki/LALR_parser) generator [YACC](https://en.wikipedia.org/wiki/Yacc) that we all were forced to learn back in [CS 440](https://cs.gmu.edu/~white/CS440/) 😅.
